...
AigGate::buildConnect(){
    ...
    else if( cirMgr->_gateList[ID(var0)]->getTypeStr()=="UNDEF")
        isflt = true;
    ...
    else if( cirMgr->_gateList[ID(var1)]->getTypeStr()=="UNDEF")
        isflt = true;
    if(isflt) cirMgr->_floatIdList.push_back(_id);
...
CirGate::simulate()
{
    ...
    if(getTypeStr() == "AIG")
    {
        if(getInInv(0))
        {
            if(getInInv(1))
            {
                _simVal = ( (~(getFanin(0)->_simVal)) & (~(getFanin(1)->_simVal)) );
            }
            else
            {
                _simVal = ( (~(getFanin(0)->_simVal)) & (getFanin(1)->_simVal));//seg
            } 
        }
        else
        {
